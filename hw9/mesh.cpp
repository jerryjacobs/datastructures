...
bool Mesh::Collapse() {
    ...
    //get the average of a and b
    assert (a->getID() < b->getID());
    a->Average(b);
    //saved triangle vertices
    std::vector<std::vector<Vertex*> > verts;
    //make copy of the triangles
    std::set<Triangle*> copy = a->getTriangles();
    copy.insert(b->getTriangles().begin(), b->getTriangles().end());
    
    for (std::set<Triangle*>::iterator itr = copy.begin(); itr != copy.end(); itr++){
        Triangle *t = *itr;
        Vertex *v1 = t->getVertex(0);
        Vertex *v2 = t->getVertex(1);
        Vertex *v3 = t->getVertex(2);
        //triangle has vertices a and b
        if (t->HasVertex(a) and t->HasVertex(b)){
            //remove edges
            RemoveEdge(v1,v2);
            RemoveEdge(v2,v3);
            RemoveEdge(v3,v1);
        ...
        //triangle has vertex a or b
        else if (t->HasVertex(a) or t->HasVertex(b)){
            //remove edges
            RemoveEdge(v1,v2);
            RemoveEdge(v2,v3);
            RemoveEdge(v3,v1);
            //if triangle has vertex b, set it equal to a
            if (v1 == b) { v1 = a; }
            else if (v2 == b) { v2 = a; }
            else if (v3 == b) { v3 = a; }
    ...
    //using saved vertices, add triangle
    for (unsigned int i = 0; i < verts.size(); i++){
        AddTriangle(verts[i][0], verts[i][1], verts[i][2]);
    }
