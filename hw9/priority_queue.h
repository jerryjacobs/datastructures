...
void PriorityQueue<T>::push(T element) {
  ...
  if (itr != locations.end()) {
    std::cout << "ERROR! priority queue already contains " << element << std::endl;
...
void PriorityQueue<T>::remove(T element) {
    ...
    itr = locations.find(element);
    //element is in map
    if (itr != locations.end()){
        int pos = locations[element]; //determine position of element in vector
        //element is not last position
        if (pos != m_heap.size()-1){
            locations.erase(element); //remove element from map
            locations[m_heap.back()] = pos; //the last element will have a new position
            m_heap[pos] = m_heap.back(); //last element replaces position of element
            m_heap.pop_back(); //remove last element
            //reposition swapped element
            percolate_up(pos);
            percolate_down(pos);
        }
        //element is last position
        else{
...
void PriorityQueue<T>::percolate_up(int i) {
    //make sure i is within vector
    while (i > 0 and i < size()){
        if (m_heap[i]->getPriorityValue() != m_heap[get_parent(i)]->getPriorityValue()) {
            //current has a smaller priority value than its parent
            if (m_heap[i]->getPriorityValue() < m_heap[get_parent(i)]->getPriorityValue()){
                //swap current with its parent in map
                locations[m_heap[get_parent(i)]] = i;
                locations[m_heap[i]] = get_parent(i);
                //swap current with its parent in vector
                T temp = m_heap[get_parent(i)];
                m_heap[get_parent(i)] = m_heap[i];
                m_heap[i] = temp;
                //update value of i
                i = get_parent(i);
            }
...
void PriorityQueue<T>::percolate_down(int i) {
    //make sure it can still percolate down
    while (has_left_child(i) or has_right_child(i)){
        
        bool swap = false;
        //swap lowest priority child with parent if one of the children is lower priority than parent
        if (has_left_child(i) and m_heap[get_left_child(i)]->getPriorityValue() < m_heap[i]->getPriorityValue()) { swap = true; }
        else if (has_right_child(i) and m_heap[get_right_child(i)]->getPriorityValue() < m_heap[i]->getPriorityValue()) { swap = true; }
        
        //parent needs to swap with lowest priority child
        if (swap){
            
            //determine lowest priority child
            int lowest;
            //only has one child
            if (!has_right_child(i)) { lowest = get_left_child(i); }
            else if (!has_left_child(i)) { lowest = get_right_child(i); }
            //has two children
            else{
                //left child has lowest priority
                if (m_heap[get_left_child(i)]->getPriorityValue() <= m_heap[get_right_child(i)]->getPriorityValue()){
                    lowest = get_left_child(i);
                }
                //right child has lowest priority
                else{
                    lowest = get_right_child(i);
                }
            }
            //swap current with lowest in map
            locations[m_heap[lowest]] = i;
            locations[m_heap[i]] = lowest;
            //swap current with lowest in vector
            T temp = m_heap[lowest];
            m_heap[lowest] = m_heap[i];
            m_heap[i] = temp;
            //update value of i
            i = lowest;
        }
