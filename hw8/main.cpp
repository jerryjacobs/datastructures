...
void student_tests() {
  std::cout << "Beginning student_tests()..." << std::endl;
    ...
    //corner cases
    QuadTree<int,char> simple;
    simple.insert(Point<int>(20,10), 'A');
    simple.insert(Point<int>(17,8), 'B');
    simple.insert(Point<int>(14,6), 'C');
    simple.insert(Point<int>(11,4), 'D');
    simple.insert(Point<int>(8,2), 'E');
    std::cout << "\nan empty tree:" << std::endl;
    simple.plot(40,20);
    
    QuadTree<int,char> single;
    single.insert(Point<int>(5,5), 'A');
    std::cout << "\nshould match:" << std::endl;
    single.print_sideways();
    QuadTree<int,char>::iterator df_itr = single.begin();
    int depth = df_itr.getDepth();
    std::cout << std::string(depth*2,' ') << df_itr.getLabel() << " " << *df_itr << std::endl;
    std::cout << "breadth:" << std::endl;
    QuadTree<int,char>::bf_iterator bf_itr = single.bf_begin();
    std::cout << bf_itr.getLabel() << " " << *bf_itr << std::endl;
    
    //other data types
    QuadTree<double,std::string> other;
    other.insert(Point<double>(18.3,9.6), "Alpha");
    other.insert(Point<double>(10.8,6.2), "Bravo");
    other.insert(Point<double>(32.2,5.0), "Charlie");
    other.insert(Point<double>(28.9,13.9), "Delta");
    other.insert(Point<double>(37.2,10.0), "Echo");
    other.print_sideways();
